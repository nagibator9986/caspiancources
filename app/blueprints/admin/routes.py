...
def save_upload(file_storage, subdir: str):
    """
    Сохраняет файл с уникальным именем и возвращает относительный путь
    (относительно UPLOAD_FOLDER), либо None.
    """
    ...
    unique_name = f"{name[:60]}_{uuid.uuid4().hex[:10]}{ext.lower()}"

    upload_root = current_app.config["UPLOAD_FOLDER"]
    dir_full = os.path.join(upload_root, subdir)
    os.makedirs(dir_full, exist_ok=True)
    full_path = os.path.join(dir_full, unique_name)
...
def certificate_branding():
    ...
    if form.validate_on_submit():
        ...
        upload_root = os.path.join(
            current_app.root_path, "static", "uploads", "certificate_branding"
        )
...
def module_create(course_id):
    ...
    if form.validate_on_submit():
        next_order = (
            db.session.query(func.coalesce(func.max(CourseModule.order_index), 0))
        ...
        module = CourseModule(
            ...
            title=form.title.data,
            description=form.description.data,
            order_index=form.order_index.data if form.order_index.data is not None else next_order + 1,
        )
...
def lesson_create(module_id):
    ...
    if form.validate_on_submit():
        next_order = (
            db.session.query(func.coalesce(func.max(Lesson.order_index), 0))
...
def lesson_edit(lesson_id):
    ...
    if form.validate_on_submit():
        ...
        if form.order_index.data is not None:
...
def lesson_block_create(lesson_id):
    ...
    if form.validate_on_submit():
        next_order = (
            db.session.query(func.coalesce(func.max(LessonBlock.order_index), 0))
        ...
        if form.block_type.data == "video":
            if form.video_url.data:
                ...
                rel_path = save_upload(
                    upload,
                    f"course_{lesson.module.course_id}/lessons/{lesson.id}/video",
                )
                if rel_path:
                    block.video_source = "upload"
                    block.video_file_path = rel_path

        elif form.block_type.data == "image" and upload:
            rel_path = save_upload(
                upload,
                f"course_{lesson.module.course_id}/lessons/{lesson.id}/images",
            )
            if rel_path:
                block.image_path = rel_path

        elif form.block_type.data == "file" and upload:
            rel_path = save_upload(
                upload,
                f"course_{lesson.module.course_id}/lessons/{lesson.id}/files",
            )
            if rel_path:
...
def question_create(test_id):
    ...
    if form.validate_on_submit():
        next_order = (
            db.session.query(func.coalesce(func.max(Question.order_index), 0))
...
def answers_manage(question_id):
    """
    Управление вариантами ответа: до 6 штук.
    Обновляем существующие, чтобы не ломать связи попыток (UserTestAttempt их не хранит,
    но всё же сохраняем стабильные id), новые — добавляем, незаполненные — удаляем.
    """
    question = Question.query.get_or_404(question_id)
    MAX_OPTIONS = 6

    if request.method == "POST":
        existing = AnswerOption.query.filter_by(question_id=question.id).all()
        existing_by_idx = list(existing) + [None] * MAX_OPTIONS  # выровнено по индексу

        for i in range(MAX_OPTIONS):
            text = (request.form.get(f"opt_text_{i + 1}") or "").strip()
            is_correct = bool(request.form.get(f"opt_correct_{i + 1}"))
            opt = existing_by_idx[i]
